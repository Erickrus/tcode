...
class TcodeBridge:
    ...
    def _subscribe_events(self) -> None:
        ...
        self._unsubs.append(
            bus.subscribe("tool.failed", self._on_tool_failed)
        )
        self._unsubs.append(
    ...
    async def _on_tool_failed(self, ev: Event) -> None:
        call_id = ev.payload.get("callID", "")
        error = ev.payload.get("error", {})
        error_msg = error.get("error", "Tool failed") if isinstance(error, dict) else str(error)
        for msg in reversed(self.state.messages):
            if msg.get("type") == "tool" and msg.get("call_id") == call_id:
                msg["status"] = "error"
                msg["output"] = {"error": error_msg}
                break
        self.app.schedule_repaint()
    ...
    async def _handle_slash_command(self, text: str) -> None:
        ...
        if cmd in ("quit", "exit"):
            ...
            # Save to tcode.json for persistence across sessions
            from ..config import save_model_to_project
            save_model_to_project(self.cli.project_dir, self.state.provider_id, self.state.model_id)
            self.state.messages.append({
                "type": "system",
                "text": f"Model: {self.state.provider_id}/{self.state.model_id} (saved)",
            })
